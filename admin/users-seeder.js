  ...
  */
  const createUsers = async (n = 20) => {
    ...
    for (let i=0; i < n;i++) {
      ...
      promises.push(await createUser(faker.internet.email(faker.name.firstName(gender), faker.name.lastName(gender)), 'reallysecurepassword123', faker.internet.displayName, `https://robohash.org/${uuidv4()}?gravatar=hashed`));
    }
